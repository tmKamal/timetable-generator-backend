...
const addLecturer = async (req, res, next) => {
  ...
  try {
    ...
    const error = new HttpError(
      "Lecturer has not created successfully, error on db",
      500
    );
    return next(error);
  }
...
const updateLecturer = async (req, res, next) => {
  ...
  res.status(200).json({
    ...
    msg: "Lecturer has updated successfully!!",
  });
};
...
const setNotAvailableTime = async (req, res, next) => {
  ...
  selectedLecturer.notAvailable.push(notAvailableTime);
  ...
  res.status(200).json({
    lecturer: selectedLecturer.toObject({ getters: true }),
    msg:
      "Not Available time has been successfully recorded in to the Selected Lecturer",
  });
};
